...
static int do_make_slave(struct vfsmount *mnt)
{
	 ...
	 */
	 //因为设置master挂载的挂载点必须先设置成shared, 所以要搜索share链表,找出master
	 //master和mnt必须在同一文件系统中
	...
	//重新初始化mnt_share链表, 将本vfsmount的mnt_share从链表中删除,并且初始化mnt_share
	...
	if (master) {
		list_for_each_entry(slave_mnt, &mnt->mnt_slave_list, mnt_slave)
			//将从挂载的链表上的元素的master全都置成找出来的那一个
			slave_mnt->mnt_master = master;
		//将当前mnt_slave挂在master的mnt_slave_list上
		list_move(&mnt->mnt_slave, &master->mnt_slave_list);
		//将mnt_slave_list中的所有链表元素尾插到master的mnt_slave_list上
 ...
 * 		cloned as a slave.
 */
//在propagation_mnt调用过程中,dest是last_dest的slaver或者是last_dest的peer
 ...
 */
/* 
 * 如果不存在共享挂载,这个函数就什么都不做.
 * dest用于遍历dest的peer及slave, src用于获取
 */
		    ...
		    struct vfsmount *source_mnt, struct list_head *tree_list)
{
	...
	for (m = propagation_next(dest_mnt, dest_mnt); m;
		...
		if (!(child = copy_tree(source, source->mnt_root, type))) {
			ret = -ENOMEM;
			//将tree_list中的元素(除链表头外),加入tmp_list和tmp_list.prev之间 
