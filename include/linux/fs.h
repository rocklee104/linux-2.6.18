...
//告诉vfs使"./","../"等路径重新生效,因为它们可能已经无效了,在nfs中有使用
...
struct inode {
    ...
    */
	...
	//文件的块数(以512byte为单位),当存在空洞时,实际块数(i_blocks*n)*i_blksize<i_size
	blkcnt_t		i_blocks;
	//文件中最后一个块的字节数
...
struct super_block {
	...
	//对超级块的已安装文件系统进行同步的标志
	int			s_need_sync_fs;
	//次级引用计数, 指被mount了多少次, 大于0, 表示s_root有效
	atomic_t		s_active;
	//指向超级块安全数据结构的指针
	void                    *s_security;
	...
	//链表元素,链表头是file_system_type中的fs_supers,用于管理同一类型文件系统
...
struct file_system_type {
	//文件系统的名称
	const char *name;
	//表明只读挂载,禁止setuid/setgid操作或者其他的微调
	int fs_flags;
	//从磁盘中读取superblock的方法
	...
	//同一文件系统可能有多个,将相同的文件系统(设备不同的)通过链表管理起来
